...
import { createStripePayment } from "@/lib/splitmates/services/stripe-service";
...
export async function POST(request: Request, context: { params: Promise<{ groupId: string }> }) {
  try {
    ...
    const input = stripePaymentSchema.parse(body);

    if (input.fromUserId !== actor.id) {
      return jsonError("You can only send payments from your own account.", 403);
    }

    const payment = await createStripePayment(groupId, actor.id, input.fromUserId, input.toUserId, input.amount);
    ...
    void logHttpAction({
      ...
      fallbackUserId: actor.id,
      actionJson: { groupId, amount: input.amount, fromUserId: input.fromUserId, toUserId: input.toUserId, method: "stripe" },
    });

    return jsonOk({ payment }, 201);
  } catch (error) {
    const message = error instanceof Error ? error.message : "Unable to create Stripe payment.";
